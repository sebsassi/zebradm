    ...
    zest::zt::ZernikeExpansionSpanOrthoGeo<const std::array<double, 2>> in, zest::zt::ZernikeExpansionSpanOrthoGeo<std::array<double, 2>> out)
{
    ...
    if (nmax > 1)
    {
        constexpr double coeff = (COORD == PlaneCoord::X) ?
            -1.0/sqrt5 : 1.0/sqrt5;
        out(0,0,0)[0] = coeff*in(1,1,1)[0];
    }
    ...
    for (std::size_t n = 1; n < nmax; ++n)
    {
        ...
        if (!parity)
        {
            constexpr double l_coeff = (COORD == PlaneCoord::X) ?
                -1.0/std::numbers::sqrt3 : 1.0/std::numbers::sqrt3;
            out_n(0,0)[0] = l_coeff*(
                    n_coeff_m*double(n)*(
                        in_nm1(1,1)[0] - std::numbers::sqrt2*in_nm1(1,1)[1])
                    + n_coeff_p*(double(n) + 3.0)*(
                        in_np1(1,1)[0] - std::numbers::sqrt2*in_np1(1,1)[1]));
        }
    ...
    {
        out_n(0,0)[0]
            = (1.0/std::sqrt(3.0))*n_coeff_m*double(nmax)*(-in_nm1(1,1)[0] + std::numbers::sqrt2*in_nm1(1,1)[1]);
    }
    ...
    for (std::size_t m = nmax - 1; m <= nmax; ++m)
    {
        const double lm_coeff_mmp = coeff_data.sqrt_n(nmax + m - 1)*coeff_data.sqrt_n(nmax + m);
    ...
    zest::zt::ZernikeExpansionSpanOrthoGeo<const std::array<double, 2>> in, zest::zt::ZernikeExpansionSpanOrthoGeo<std::array<double, 2>> out)
{
    ...
    for (std::size_t n = 2; n < nmax - 1; ++n)
    {
        ...
        for (std::size_t l = parity; l <= n - 2; l += 2)
        {
            ...
            const double _2lm1_sq = _2lm1*_2lm1;
    ...
    for (std::size_t n = nmax - ((nmax == 2) ? 0 : 1); n <= nmax; ++n)
    {
        ...
        auto out_nmaxm1 = out[nmax - 1];
        auto in_nmaxm3 = in[nmax - 1];
        auto in_nmaxm1 = in[nmax - 1];
