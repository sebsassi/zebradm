...
\end{minted}
From there we want to use the class \cppinline{ZernikeTransformer} as our transformer. We first need to initialize the transformer. There are two options for this, we can default initialize it
\begin{minted}{c++}
    zest::zt::ZernikeTransformer<zest::zt::NormedGeo> zernike_transformer();
\end{minted}
or we can initialize it with a \texttt{order} parameter
\begin{minted}{c++}
    zest::zt::ZernikeTransformer<zest::zt::NormedGeo> zernike_transformer(order);
\end{minted}
...
\begin{minted}{c++}
    ...
    zdm::ZernikeExpansion<double> zernike_expansion
